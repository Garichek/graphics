...
public class JacksonUtils {
    ...
    private static ObjectMapper xmlMapper = getXmlMapper();

    private static ObjectMapper fullMapper = createMapper().disable(MapperFeature.USE_ANNOTATIONS);
...
//		objectMapper.configure(MapperFeature.ALLOW_COERCION_OF_SCALARS, false); For preventing string to integer auto conversion
    ...
    public static <T> T fromJsonFull(Class<T> clazz, File file) {
        try {
            T value = fullMapper.readValue(file, clazz);
            return value;
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static <T> T fromJsonFull(Class<T> clazz, String json) {
        try {
            T value = fullMapper.readValue(json, clazz);
            return value;
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static String getXml(Object object) {
        try {
            ...
            log.error("Cannot get xml from the object %s", object);
        }
        return null;
    }
    ...
    public static <T> T fromXml(Class<T> clazz, String json) {
        try {
            return xmlMapper.readValue(json, clazz);
        } catch (IOException e) {
            log.error("Cannot read object %s from a xml %s", clazz, json);
            log.error("", e);
        }
        return null;
    }
